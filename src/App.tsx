...
function WorkspaceReady({ auth, gateway, planGateway, planRecordGateway, planMemoryGateway, researchGateway, achievementGateway, todayGateway, todayOutbox, claraGateway, claraApprovalGateway, scheduleRunGateway, approvedDayGateway, dayBreakGateway, releaseSurface }: {
  ...
  if (stage === 'plan-details') {
    ...
    return <main className="app-shell plan-details-shell">
      <header><p className="eyebrow">Longview</p><button className="secondary compact" onClick={() => { clara.cancel(); setShowPlanClara(false); setApprovalProposal(null); setGuidanceReview(null); setStage('today'); setView('plans'); }}>Back to Plans</button></header>
  ...
  if (stage === 'plan-saved') {
    return <main className="shell"><p className="eyebrow">Longview</p><section className="card success"><span className="status">Plan saved</span><h1>{planDraft.title.trim()}</h1><p>Your Plan is ready. Longview will use it to shape your next useful step.</p><button onClick={() => { setView('today'); setStage('today'); }}>Return to Today</button></section></main>;
  }

  if (stage === 'today') {
    return <main className="app-shell"><header><p className="eyebrow">Longview</p><span className="status">{phaseZero ? 'Early access' : releaseSurface === 'release-one' ? 'Ask Clara · read only' : releaseSurface === 'release-two' ? 'Clara changes · review first' : releaseSurface === 'release-three' ? 'Daily schedule · review first' : releaseSurface === 'release-five' ? 'Research + Plan Brief' : releaseSurface === 'release-six' ? 'Achievement + consent' : 'Plan-based schedules'}</span></header>
      ...
      <nav aria-label="Primary"><button aria-current={view === 'today' ? 'page' : undefined} className={view === 'today' ? '' : 'secondary'} onClick={() => setView('today')}>Today</button>{dailyScheduleEnabled && <button aria-current={view === 'calendar' ? 'page' : undefined} className={view === 'calendar' ? '' : 'secondary'} onClick={() => setView('calendar')}>Calendar</button>}<button aria-current={view === 'plans' ? 'page' : undefined} className={view === 'plans' ? '' : 'secondary'} onClick={() => setView('plans')}>Plans</button><button aria-current={view === 'settings' ? 'page' : undefined} className={view === 'settings' ? '' : 'secondary'} onClick={() => setView('settings')}>Settings</button></nav></main>;
  }
