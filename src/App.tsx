...
function ScheduleRunPanel({ scheduleRun, capacityMinutes, eligibleCount, planCount, onCapacity, onStart, onReturn, onCreatePlan, onReviewPlans }: {
...
function WorkspaceReady({ auth, gateway, planGateway, todayGateway, claraGateway, claraApprovalGateway, scheduleRunGateway }: {
  ...
  if (stage === 'today') {
    return <main className="app-shell"><header><p className="eyebrow">Longview</p><span className="status">Plan-based schedules</span></header>
      ...
      {view === 'calendar' && <ScheduleRunPanel scheduleRun={scheduleRun} capacityMinutes={planningWindow} eligibleCount={scheduleRunContext?.steps.length ?? 0} planCount={portfolio.entries.length} onCapacity={setPlanningWindow} onStart={startScheduleRun} onReturn={() => { scheduleRun.reset(); setView('today'); }} onCreatePlan={() => { scheduleRun.reset(); startNewPlan(); }} onReviewPlans={() => { scheduleRun.reset(); setView('plans'); }} />}
