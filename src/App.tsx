...
import { lazyFirebasePlanMemoryGateway } from './planMemory/lazyGateway';
...
import { lazyPlanMatchGateway } from './planResearch/lazyMatchGateway';
...
import { AppNavigation, sectionForView, type PrimarySection } from './navigation/AppNavigation';
import './styles.css';

export type ReleaseSurface = 'phase-zero' | 'release-one' | 'release-two' | 'release-three' | 'release-four' | 'release-five' | 'release-six' | 'full';
...
const validReleaseSurface = (value: unknown): value is ReleaseSurface =>
  value === 'phase-zero' || value === 'release-one' || value === 'release-two' || value === 'release-three' || value === 'release-four' || value === 'release-five' || value === 'release-six' || value === 'full';
...
const failureCopy: Record<AuthFailure, string> = {
  ...
  'popup-blocked': 'Google sign-in could not open. Try again, or continue without Google.',
...
function ClaraApprovalPanel({ proposal, state, onApprove, onReject, onReturn, returnLabel = 'View updated Today' }: {
  ...
  const copy = snapshot.failure === 'future-approved'
    ...
    : snapshot.failure === 'no-eligible-day'
      ...
      : snapshot.failure === 'source-changed'
        ...
        : snapshot.failure === 'timeout'
          ? ['Saving took too long.', 'The request stopped at its deadline. Today’s approved order is still available; a retry restores a completed break safely.']
        : ['The break was not saved.', 'Today’s approved order is still available. No future day changed.'];
  return <section className="calendar-view" role="alert"><span className="status">Nothing changed</span><h1>{copy[0]}</h1><p>{copy[1]}</p><div className="actions">{(snapshot.failure === 'unavailable' || snapshot.failure === 'timeout') && snapshot.preview ? <button onClick={dayBreak.retry}>Try again</button> : snapshot.failure === 'unavailable' || snapshot.failure === 'timeout' ? <button onClick={() => dayBreak.preview(day)}>Try again</button> : snapshot.failure === 'source-changed' ? <button onClick={onReload}>Review latest day</button> : snapshot.failure === 'no-eligible-day' ? <button onClick={onReviewPlans}>Review Plan schedules</button> : null}<button className="secondary" onClick={onCancel}>Keep today’s order</button></div></section>;
}
...
function ScheduleRunPanel({ scheduleRun, approvedDay, dayBreak, capacityMinutes, eligibleCount, scheduledCount, planCount, preparationStatus, onCapacity, onStart, onReset, onReturn, onCreatePlan, onReviewPlans, onRetryPreparation }: {
  ...
  if (approvedDay.approval.status === 'error') {
    ...
    const timedOut = approvedDay.approval.failure === 'timeout';
    return <section className="calendar-view" role="alert"><span className="status">Nothing changed</span><h1>{conflict ? 'This proposal is out of date.' : timedOut ? 'Saving took too long.' : 'Today was not changed.'}</h1><p>{conflict ? 'The approved day changed after this proposal was prepared. Review the latest day before preparing another replacement.' : timedOut ? 'The request stopped at its deadline. Your previously approved day is still available; retry restores a completed approval safely.' : 'Longview could not save this order. Your previously approved day is still available.'}</p><div className="actions">{conflict ? <button onClick={() => { onReset(); void approvedDay.reload(); }}>Review latest day</button> : <button onClick={approvedDay.retryApproval}>Try approval again</button>}<button className="secondary" onClick={onReset}>Keep approved day</button></div></section>;
  }
  ...
  const timedOut = snapshot.status === 'timed-out' || (snapshot.status === 'error' && snapshot.failure === 'timeout');
  const malformed = snapshot.status === 'error' && snapshot.failure === 'malformed';
...
function WorkspaceReady({ auth, gateway, planGateway, planRecordGateway, planMemoryGateway, planResearchSourceGateway, planMatchGateway, researchGateway, achievementGateway, todayGateway, todayOutbox, claraGateway, claraApprovalGateway, scheduleRunGateway, approvedDayGateway, dayBreakGateway, releaseSurface }: {
  ...
  planMatchGateway: PlanMatchGateway;
  ...
  const achievementEnabled = releaseSurface === 'release-six' || extendedAgentFeatures;
  const planMemoryEnabled = releaseSurface === 'release-five' || achievementEnabled;
  const planRecordEnabled = releaseSurface === 'release-four' || planMemoryEnabled;
  ...
  const [showPlanClara, setShowPlanClara] = useState(false);
  ...
  const completedPlans = useMemo(() => plans.snapshot.plans.filter(plan => plan.status === 'completed'), [plans.snapshot.plans]);
  ...
  const approvedDay = useApprovedDay(
    approvedDayGateway,
    localDate(),
    dailyScheduleEnabled && stage === 'today' && (view === 'today' || view === 'calendar')
  );
  const todayBreakDay = approvedDay.snapshot.status === 'ready' && approvedDay.snapshot.day?.status === 'break'
    ? approvedDay.snapshot.day
    : null;
  const todayScheduleReady = !dailyScheduleEnabled || approvedDay.snapshot.status === 'ready';
  const completion = useTodayCompletion(
    snapshot.user,
    todayStep,
    todayGateway,
    todayOutbox,
    stage === 'today' && view === 'today' && plans.snapshot.status === 'ready' && todayScheduleReady && !todayBreakDay
  );
  const clara = useClaraRecommendation(claraGateway, 18000, approvedClaraChanges);
  const scheduleRun = useScheduleRun(scheduleRunGateway);
  const dayBreak = useDayBreak(dayBreakGateway);

  const navigateToSection = (section: PrimarySection) => {
    clara.cancel();
    setShowClara(false);
  ...
  const askClaraAboutPlan = (plan: Plan) => {
    setShowPlanClara(true);
  ...
  const closeClaraApproval = () => {
    ...
    setShowPlanClara(false);
    plans.retry();
    if (stage === 'plan-details') planDetails.retry();
  };
  ...
  if (stage === 'plan-details') {
    ...
    if (detailSnapshot.status !== 'ready') {
      const returnToPlans = () => { navigateToSection('plans'); plans.retry(); };
      if (detailSnapshot.status === 'idle' || detailSnapshot.status === 'loading') return <AppNavigation section="plans" screen="Plan details" breadcrumb="Plans / Plan details" backLabel="Back to Plans" onBack={() => navigateToSection('plans')} calendarEnabled={dailyScheduleEnabled} onNavigate={navigateToSection}><section className="shell embedded-shell" aria-busy="true"><section className="card"><span className="status">Plan details</span><h1>Loading this Plan…</h1><p>Reading the latest owner-approved version.</p></section></section></AppNavigation>;
      if (detailSnapshot.status === 'missing') return <AppNavigation section="plans" screen="Plan unavailable" breadcrumb="Plans / Plan details" backLabel="Back to Plans" onBack={returnToPlans} calendarEnabled={dailyScheduleEnabled} onNavigate={navigateToSection}><section className="shell embedded-shell"><section className="card"><span className="status">Plan unavailable</span><h1>This Plan is no longer available.</h1><p>No old details are shown. Return to Plans to load the latest portfolio.</p><button onClick={returnToPlans}>Return to Plans</button></section></section></AppNavigation>;
      return <AppNavigation section="plans" screen="Plan unavailable" breadcrumb="Plans / Plan details" backLabel="Back to Plans" onBack={() => navigateToSection('plans')} calendarEnabled={dailyScheduleEnabled} onNavigate={navigateToSection}><section className="shell embedded-shell"><section className="card"><span className="status">Plan details unavailable</span><h1>Couldn’t load this Plan.</h1><p>The saved Plan is unchanged. Check your connection and retry the same Plan.</p><div className="actions"><button onClick={planDetails.retry}>Try again</button><button className="secondary" onClick={() => navigateToSection('plans')}>Return to Plans</button></div></section></section></AppNavigation>;
    }
    const plan = detailSnapshot.plan;
    const mode = portfolio.entries.find(entry => entry.plan.id === plan.id)?.mode ?? 'Focus';
    return <AppNavigation section="plans" screen="Plan details" breadcrumb={`Plans / ${plan.title}`} backLabel="Back to Plans" onBack={() => navigateToSection('plans')} calendarEnabled={dailyScheduleEnabled} onNavigate={navigateToSection}>
      <section className="plan-details-view">
        <span className="status">{plan.status === 'completed' ? 'Plan achieved' : `Plan details · ${mode}`}</span><h1>{plan.title}</h1><p className="lead">{plan.outcome}</p>
        <div className="detail-grid">
          <article className="plan-card"><h2>Plan overview</h2><dl><dt>Why it matters</dt><dd>{plan.why}</dd><dt>Target date</dt><dd>{formatLongDate(plan.targetDate)}</dd><dt>Weekly time</dt><dd>{plan.weeklyHours} hours</dd><dt>Working days</dt><dd>{plan.workingDays ? orderWorkingDays(plan.workingDays).map(day => dayLabels[day]).join(', ') : 'Schedule not set'}</dd></dl>{plan.status === 'active' && <button onClick={() => openPlanSchedule(plan)}>{plan.workingDays ? 'Edit schedule' : 'Add schedule'}</button>}</article>
          <article className="plan-card"><span className="status">{plan.status === 'completed' ? 'Completed Plan' : 'Current step'}</span>{plan.status === 'completed' ? <><h2>Outcome recorded.</h2><p>This Plan no longer adds work to Today or Calendar.</p><small>{plan.completedAt ? `Finished ${formatLongDate(plan.completedAt.slice(0, 10))}` : 'Completion evidence is shown below.'}</small></> : selectedDetailsStep ? <><h2>{selectedDetailsStep.title}</h2><p>{selectedDetailsStep.description}</p><small>{selectedDetailsStep.durationMinutes} minutes · scheduled today</small><button onClick={() => { clara.cancel(); setShowPlanClara(false); setApprovalProposal(null); setStage('today'); setView('today'); }}>Open Today</button></> : <><h2>Nothing scheduled today.</h2><p>{plan.workingDays ? selectedDetailsNextDate ? `This Plan returns ${formatLongDate(selectedDetailsNextDate)}.` : 'No upcoming working day is available.' : 'Add working days in Plan overview before this Plan can appear in Today.'}</p></>}</article>
        </div>
        ...
        {planMemoryEnabled && <PlanResearchSourcesSection user={snapshot.user} plan={plan} plans={plans.snapshot.plans} gateway={planResearchSourceGateway} matchGateway={planMatchGateway} />}
  ...
  if (stage === 'plan-schedule') {
    if (!selectedDetailsPlan) return <AppNavigation section="plans" screen="Plan schedule" breadcrumb="Plans / Plan schedule" backLabel="Back to Plans" onBack={() => navigateToSection('plans')} calendarEnabled={dailyScheduleEnabled} onNavigate={navigateToSection}><section className="shell embedded-shell"><section className="card"><h1>This Plan could not be found.</h1><button onClick={() => { navigateToSection('plans'); plans.retry(); }}>Return to Plans</button></section></section></AppNavigation>;
    return <AppNavigation section="plans" screen="Plan schedule" breadcrumb={`Plans / ${selectedDetailsPlan.title} / Schedule`} backLabel="Back to Plan" onBack={() => setStage('plan-details')} calendarEnabled={dailyScheduleEnabled} onNavigate={navigateToSection}><section className="shell embedded-shell"><section className="card"><span className="status">Plan schedule</span><h1>When will you work on {selectedDetailsPlan.title}?</h1><p>Choose working days and a weekly allocation. Clock times can be added later.</p><fieldset><legend>Working days</legend><div className="day-choices">{workingDays.map(day => <button type="button" key={day} aria-pressed={scheduleDraft.workingDays.includes(day)} className={scheduleDraft.workingDays.includes(day) ? '' : 'secondary'} onClick={() => { setScheduleDraft(current => ({ ...current, workingDays: current.workingDays.includes(day) ? current.workingDays.filter(value => value !== day) : [...current.workingDays, day] })); setScheduleErrors(current => ({ ...current, workingDays: undefined })); }}>{dayLabels[day]}</button>)}</div></fieldset>{scheduleErrors.workingDays && <small role="alert">{scheduleErrors.workingDays}</small>}<label>Hours for this Plan each week<input type="number" min="1" max="40" value={scheduleDraft.weeklyHours} onChange={event => { setScheduleDraft(current => ({ ...current, weeklyHours: Number(event.target.value) })); setScheduleErrors(current => ({ ...current, weeklyHours: undefined })); }} /></label>{scheduleErrors.weeklyHours && <small role="alert">{scheduleErrors.weeklyHours}</small>}{scheduleFailure === 'unavailable' && <div className="notice" role="alert">The schedule wasn’t saved. The existing Plan is unchanged. Check your connection and try again.</div>}{scheduleFailure === 'conflict' && <div className="notice" role="alert">This Plan was updated in another session. Your changes weren’t saved. Reload the Plan before editing again.<button onClick={() => { planDetails.retry(); setStage('plan-details'); }}>Reload Plan</button></div>}<div className="actions"><button onClick={savePlanSchedule} disabled={savingSchedule}>{savingSchedule ? 'Saving schedule…' : 'Save schedule'}</button><button className="secondary" onClick={() => setStage('plan-details')} disabled={savingSchedule}>Cancel</button></div></section></section></AppNavigation>;
  }

  if (stage === 'plan-create') {
    return <AppNavigation section="plans" screen="Create Plan" breadcrumb="Plans / Create Plan" backLabel="Back to Plans" onBack={() => navigateToSection('plans')} calendarEnabled={dailyScheduleEnabled} onNavigate={navigateToSection}><section className="shell embedded-shell"><section className="card"><span className="status">Create a Plan</span><h1>What do you want to make progress on?</h1><p>Start with one meaningful outcome. You can refine it later.</p><form onSubmit={event => { event.preventDefault(); reviewPlan(); }} noValidate>
    ...
    </form></section></section></AppNavigation>;
  }

  if (stage === 'plan-review') {
    return <AppNavigation section="plans" screen="Review Plan" breadcrumb="Plans / Create Plan / Review" backLabel="Back to editing" onBack={() => setStage('plan-create')} calendarEnabled={dailyScheduleEnabled} onNavigate={navigateToSection}><section className="shell embedded-shell"><section className="card"><span className="status">Review your Plan</span><h1>{planDraft.title.trim()}</h1><dl><dt>Desired outcome</dt><dd>{planDraft.outcome.trim()}</dd><dt>Why it matters</dt><dd>{planDraft.why.trim()}</dd><dt>Target date</dt><dd>{formatLongDate(planDraft.targetDate)}</dd><dt>Working days</dt><dd>{orderWorkingDays(planDraft.workingDays).map(day => dayLabels[day]).join(', ')}</dd><dt>Weekly time</dt><dd>{planDraft.weeklyHours} hours</dd></dl>{planSaveFailed && <div className="notice" role="alert">Your Plan wasn’t saved. Check your connection and try again.</div>}<div className="actions"><button onClick={savePlan} disabled={savingPlan}>{savingPlan ? 'Saving Plan…' : 'Create Plan'}</button><button className="secondary" onClick={() => setStage('plan-create')} disabled={savingPlan}>Edit</button><button className="secondary" onClick={() => navigateToSection('plans')} disabled={savingPlan}>Cancel</button></div></section></section></AppNavigation>;
  }

  if (stage === 'plan-saved') {
    return <AppNavigation section="plans" screen="Plan created" breadcrumb="Plans / Plan created" backLabel="View Plans" onBack={() => navigateToSection('plans')} calendarEnabled={dailyScheduleEnabled} onNavigate={navigateToSection}><section className="shell embedded-shell"><section className="card success"><span className="status">Plan saved</span><h1>{planDraft.title.trim()}</h1><p>Your Plan is ready. Longview will use it to shape your next useful step.</p><button onClick={() => navigateToSection('today')}>Return to Today</button></section></section></AppNavigation>;
  }

  if (stage === 'today') {
    const section = sectionForView(view);
    const screenTitle = view === 'clara-home' ? 'Ask Clara' : view === 'clara-actions' ? 'Quick Actions' : view === 'clara-action-detail' ? 'Action details' : section === 'today' ? 'Today' : section === 'calendar' ? 'Calendar' : section === 'plans' ? 'Plans' : 'Settings';
    const breadcrumb = view === 'clara-actions' ? 'Today / Ask Clara / Quick Actions' : view === 'clara-action-detail' ? 'Today / Ask Clara / Quick Actions / Action details' : view === 'clara-home' ? 'Today / Ask Clara' : undefined;
    const backLabel = view === 'clara-action-detail' ? 'Back to Quick Actions' : view === 'clara-actions' ? 'Back to Clara' : view === 'clara-home' ? 'Back to Today' : undefined;
    const onBack = view === 'clara-action-detail' ? () => setView('clara-actions') : view === 'clara-actions' ? () => setView('clara-home') : view === 'clara-home' ? () => setView('today') : undefined;
    return <AppNavigation section={section} screen={screenTitle} breadcrumb={breadcrumb} backLabel={backLabel} onBack={onBack} calendarEnabled={dailyScheduleEnabled} onNavigate={navigateToSection}>
      {view === 'today' && <section className="today-view" aria-busy={plans.snapshot.status === 'idle' || plans.snapshot.status === 'loading' || (dailyScheduleEnabled && (approvedDay.snapshot.status === 'idle' || approvedDay.snapshot.status === 'loading'))}><span className="status">Today</span>
        {dailyScheduleEnabled && (approvedDay.snapshot.status === 'idle' || approvedDay.snapshot.status === 'loading') && <div className="empty"><h1>Checking today’s schedule…</h1><p>Making sure you see the latest saved day.</p></div>}
        {dailyScheduleEnabled && approvedDay.snapshot.status === 'error' && <div className="empty" role="alert"><h1>Today’s schedule couldn’t be checked.</h1><p>No task is shown until Longview can confirm whether today is a work day or a break.</p><button onClick={() => void approvedDay.reload()}>Try again</button></div>}
        ...
        {todayScheduleReady && !todayBreakDay && (plans.snapshot.status === 'idle' || plans.snapshot.status === 'loading') && <div className="empty"><h1>Preparing Today…</h1><p>Finding one useful step from your saved Plans.</p></div>}
        {todayScheduleReady && !todayBreakDay && plans.snapshot.status === 'error' && <div className="empty"><h1>Today couldn’t be prepared.</h1><p>Your Plans are unchanged. Check your connection and try again.</p><button onClick={plans.retry}>Try again</button></div>}
        {todayScheduleReady && !todayBreakDay && plans.snapshot.status === 'ready' && !todayStep && plans.snapshot.plans.length === 0 && <div className="empty"><h1>Nothing is scheduled yet.</h1><p>Create your first Plan and choose the days you want to work on it.</p><button onClick={startNewPlan}>Create first Plan</button></div>}
        {todayScheduleReady && !todayBreakDay && plans.snapshot.status === 'ready' && !todayStep && plans.snapshot.plans.length > 0 && plans.snapshot.plans.some(plan => !plan.workingDays) && <div className="empty"><h1>A Plan needs a schedule.</h1><p>Open the Plan and add at least one working day before it can appear in Today.</p><button onClick={() => setView('plans')}>View Plans</button></div>}
        {todayScheduleReady && !todayBreakDay && plans.snapshot.status === 'ready' && !todayStep && plans.snapshot.plans.length > 0 && plans.snapshot.plans.every(plan => plan.workingDays) && <div className="empty"><h1>Nothing scheduled today.</h1><p>{nextScheduledDate ? `Your next scheduled Plan day is ${formatLongDate(nextScheduledDate)}.` : 'No active Plan has an upcoming working day.'}</p><button onClick={() => setView('plans')}>View Plans</button></div>}
        {todayScheduleReady && !todayBreakDay && plans.snapshot.status === 'ready' && todayStep && completion.snapshot.status === 'ready' && completion.snapshot.completion && <div className="today-content"><h1>{completion.snapshot.duplicate ? 'Progress already saved.' : 'Today’s step is complete.'}</h1><p>{completion.snapshot.duplicate ? 'That progress was already saved. No second completion was added and your Plan was not changed.' : 'You recorded meaningful progress without changing your Plan.'}</p><TodayStepCard step={todayStep} completed /><div className="notice"><strong>{completion.snapshot.duplicate ? 'One completion record remains.' : 'Completion saved.'}</strong><p>Completion record: {completion.snapshot.completion.id}</p></div><div className="actions">{extendedAgentFeatures && <button className="secondary" onClick={() => setView('clara-home')}>Ask Clara</button>}<button className="secondary" onClick={() => setView('plans')}>View all Plans</button></div></div>}
        {todayScheduleReady && !todayBreakDay && plans.snapshot.status === 'ready' && todayStep && !completion.snapshot.completion && completion.pending && (completion.syncStatus === 'pending' || completion.syncStatus === 'syncing' || completion.syncStatus === 'retry') && <PendingTodayCompletion step={todayStep} syncStatus={completion.syncStatus} offline={completion.offline} onRetry={() => void completion.retrySync()} />}
        {todayScheduleReady && !todayBreakDay && plans.snapshot.status === 'ready' && todayStep && !completion.snapshot.completion && completion.syncStatus === 'blocked' && <div className="today-content"><h1>This completion needs attention</h1><p>Longview could not safely match the saved progress to this workspace, so it was not added or replaced.</p><TodayStepCard step={todayStep} /><div className="notice" role="alert"><strong>Your Plan is unchanged.</strong><p>Check that you’re using the same account, then try again.</p></div>{completion.pending && <button onClick={() => void completion.retrySync()} disabled={completion.offline}>{completion.offline ? 'Reconnect to try again' : 'Try sync again'}</button>}</div>}
        {todayScheduleReady && !todayBreakDay && plans.snapshot.status === 'ready' && todayStep && completion.snapshot.status !== 'ready' && <div className="today-content"><h1>One useful step is enough.</h1><p>Start with the nearest active Plan. You can refine the step later.</p><TodayStepCard step={todayStep} />{completion.snapshot.status === 'error' ? <div className="notice" role="alert">Progress couldn’t be checked. Nothing was changed.<button onClick={completion.retry}>Try again</button></div> : <button disabled>Checking progress…</button>}</div>}
        {todayScheduleReady && !todayBreakDay && plans.snapshot.status === 'ready' && todayStep && completion.snapshot.status === 'ready' && !completion.snapshot.completion && !completion.pending && completion.syncStatus !== 'blocked' && <div className="today-content"><h1>One useful step is enough.</h1><p>Start with the nearest active Plan. You can refine the step later.</p><TodayStepCard step={todayStep} />{claraEnabled && (approvedClaraChanges && approvalProposal ? <ClaraApprovalPanel proposal={approvalProposal} state={approvalState} onApprove={applyClaraChange} onReject={closeClaraApproval} onReturn={closeClaraApproval} /> : showClara ? <ClaraPanel clara={clara} onReview={approvedClaraChanges ? reviewClaraChange : undefined} onClose={() => { clara.cancel(); setShowClara(false); }} /> : <div className="actions">{extendedAgentFeatures && <button className="secondary" onClick={() => setView('clara-home')}>Ask Clara</button>}<button className="secondary" onClick={askClara}>Ask Clara about this step</button></div>)}{completion.saveFailed && <div className="notice" role="alert">Completion wasn’t saved on this device. Your step is still open. Try again.</div>}{confirmComplete ? <div className="notice" role="alert"><p>{completion.offline ? 'Save this completion on your device? Longview will sync it after your connection returns.' : 'Mark this step complete for today? Your Plan and schedule will stay the same.'}</p><div className="actions"><button onClick={async () => { if (await completion.complete()) setConfirmComplete(false); }} disabled={completion.completing}>{completion.completing ? 'Saving completion…' : completion.offline ? 'Save on this device' : 'Confirm completion'}</button><button className="secondary" onClick={() => setConfirmComplete(false)} disabled={completion.completing}>Keep working</button></div></div> : <div className="actions"><button onClick={() => setConfirmComplete(true)}>Mark step complete</button><button className="secondary" onClick={() => setView('plans')}>View all Plans</button></div>}</div>}
      </section>}
      {extendedAgentFeatures && view === 'clara-home' && <ClaraHome planCount={portfolio.entries.length} taskCount={calendarSteps.length} onOpenActions={() => setView('clara-actions')} onAskAboutStep={askClaraFromHub} onReturn={() => setView('today')} />}
      {extendedAgentFeatures && view === 'clara-actions' && <ClaraQuickActions onOpenGroup={group => { setQuickActionGroup(group); setView('clara-action-detail'); }} onReturn={() => setView('clara-home')} />}
      {extendedAgentFeatures && view === 'clara-action-detail' && <ClaraQuickActionDetail groupId={quickActionGroup} onChoose={openQuickActionTarget} onReturn={() => setView('clara-actions')} />}
  ...
  return (
    <main className="shell">
      ...
      <section className="card success">
        ...
        {snapshot.user.isAnonymous && <div className="actions"><button onClick={continueSetup}>Continue setup</button><button className="secondary" onClick={auth.linkGoogle} disabled={snapshot.linking}>{snapshot.linking ? 'Taking you to Google…' : 'Link Google account'}</button></div>}
...
export function App({ gateway = firebaseAuthGateway, workspaceGateway = lazyFirebaseWorkspaceGateway, planGateway = lazyFirebasePlanGateway, planRecordGateway = lazyFirebasePlanRecordGateway, planMemoryGateway = lazyFirebasePlanMemoryGateway, planResearchSourceGateway = lazyFirebasePlanResearchSourceGateway, planMatchGateway = lazyPlanMatchGateway, researchGateway = lazyResearchGateway, achievementGateway = lazyAchievementGateway, todayGateway = lazyFirebaseTodayGateway, todayOutbox = lazyIndexedDbTodayOutbox, claraGateway = lazyClaraGateway, claraApprovalGateway = lazyClaraApprovalGateway, scheduleRunGateway = lazyScheduleRunGateway, approvedDayGateway = lazyApprovedDayGateway, dayBreakGateway = lazyDayBreakGateway, releaseSurface = defaultReleaseSurface }: { gateway?: AuthGateway; workspaceGateway?: WorkspaceGateway; planGateway?: PlanGateway; planRecordGateway?: PlanRecordGateway; planMemoryGateway?: PlanMemoryGateway; planResearchSourceGateway?: PlanResearchSourceGateway; planMatchGateway?: PlanMatchGateway; researchGateway?: ResearchGateway; achievementGateway?: AchievementGateway; todayGateway?: TodayGateway; todayOutbox?: TodayOutbox; claraGateway?: ClaraGateway; claraApprovalGateway?: ClaraApprovalGateway; scheduleRunGateway?: ScheduleRunGateway; approvedDayGateway?: ApprovedDayGateway; dayBreakGateway?: DayBreakGateway; releaseSurface?: ReleaseSurface }) {
